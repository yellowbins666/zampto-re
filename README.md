...
| `NODE_LINK` | ⬜ 可选 | 空 | **CI 专用 Secret**：sing-box 代理节点链接（hy2/vless/anytls 等），用于搭建本地出口 |
...
- `true`（默认）：浏览器通过本地 sing-box 代理访问 ZamPTO，适用于需要特定出口 IP 才能访问/注册的地区。
- `false`：直连，不使用任何代理。
...
### `NODE_LINK` — 代理节点链接（CI 专用 Secret）
这是 **sing-box 的真实出口节点链接**，仅用于 **GitHub Actions**（作为 Repo Secret 配置，并非 `renew.py` 直接读取的普通环境变量）。支持协议包括但不限于：

- `hysteria2://`（hy2）
- `vless://`
- `anytls://`

Workflow 在「设置 sing-box 代理」步骤中把 `NODE_LINK` 传给 `setup_proxy.sh`，由其生成 sing-box 配置并监听本地端口 `127.0.0.1:1081`；脚本再把浏览器流量交给该本地端口，由 sing-box 转发到你的节点。

```bash
# 仅 CI（Settings → Secrets）中配置，例如：
NODE_LINK="hysteria2://user:pass@host:port/?...&sni=..."
```

> 🔑 **本地运行**时无需此变量：你自己启动的 sing-box（监听 `127.0.0.1:1081`）导入了对应节点链接即可；脚本默认就指向该本地端口。
...
| `NODE_LINK` | — | 见上方主表格 `NODE_LINK` 行（sing-box 代理节点链接，CI 专用） |
...
3. **代理可靠性**：`IS_PROXY=true` 时务必保证本地 sing-box 代理（`127.0.0.1:1081`）可达，否则启动即失败并推送错误通知。
